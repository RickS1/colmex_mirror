$(document).on("scroll", function(event) {
  ...
  if (y > 200 || barra_render) {
		...
		$("#div_logo a").stop().animate({"font-size": 0}, 50);
...
function ajustarCuerpo(scroll){
  var ancho_act = Math.max(document.documentElement.clientWidth, window.innerWidth || document.body.ClientWidth || 0);
	...
	$('#resto-cuerpo').css({'padding-bottom': (alto_footer+1)+'px', 'padding-top': /*(alto_header-1.5+(scroll > 200 ? 170 : 0))*/ (ancho_act <= 1165 ? alto_header : '230') + 'px'});
}
